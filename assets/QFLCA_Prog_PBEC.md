...
#### **Why these values?**
*   **The $\sqrt{2}$ Balance**: This is the "Unitary Threshold" at this value $|\kappa^2| = 2$ for back and forth distance unit of the interaction length between one or more entangled particled particle pairs. For the system to remain physically real ($ \le 2 $) which is meaured relative to the Planck distance as the uncertaintity is expected, the coherence factor **$\rho$** must be **1**. This represents the most efficient state of a quantum (QDF) heat engine.
